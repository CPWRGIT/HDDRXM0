...
*********
       LINKAGE SECTION.
       01  EOM-SW                     PIC X.
           88  FIRST-EXECUTION      VALUES LOW-VALUES.
...
*********  FEB REQUIRES SPECIAL HADNLING BECAUSE OF LEAP YEARS
*********
         2000-CALC-END-OF-MONTH.
             IF RUN-MM = 02
                 PERFORM 3000-HANDLE-FEB
             ELSE
                 IF DATE-DD(RUN-MM) = RUN-DD
                     MOVE 'Y' TO EOM-SW.
*********
*********  FOR FEBRUARY, DETERMINE IF THE RUN YEAR IS A LEAP YEAR.
*********  IF SO AND RUN DAY IS 29TH IT'S END OF MONTH
